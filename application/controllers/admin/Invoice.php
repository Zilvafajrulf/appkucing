...
class Invoice extends CI_Controller
{
	...
	{
		$data['title'] = 'Invoice';
		$id_shelter = $this->session->userdata('id_shelter');
        $data['shelter'] = $this->db->query("SELECT * FROM shelter 
			WHERE shelter.id_shelter='$id_shelter'")->result();
		$data['invoice'] = $this->model_invoice->get($id_shelter);
	...
	{
		$this->db->update('transaction', ['status' => '2'], ['order_id' => $id]);
		$_SESSION["sukses"] = 'Pesanan berhasil dibatalkan';
		redirect('admin/invoice');
	}
