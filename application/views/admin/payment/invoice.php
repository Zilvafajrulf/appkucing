                    ...
                    <tbody>
                        ...
                        foreach ($invoice as $row) : ?>
                            <tr class="intro-x">
                                ...
                                <td>
                                  <?php if ($row->status == "0"){ ?>
                                    ...
                                    <?php }  else if ($row->status == "2"){ ?>
                                        <div class="flex items-center whitespace-nowrap text-danger"> <b>CANCEL</b> </div>
                                    <?php } ?>
                                </td>

                                <!-- else ($row->status == "2")?>
                                        <div class="flex items-center whitespace-nowrap text-success"> <b>CANCEL</b> </div> -->

                                <td class="table-report__action"><center>
                                <div>
                                <?php if ($row->status == "0"){ ?>
                                    <div class="flex justify-center items-center">
                                    <a href="<?= site_url('admin/invoice/confirm/'. $row->order_id)?>" class="btn btn-sm btn-success text-white">Confirm</a>
                                    </div>   
                                <?php } else if ($row->status == "1") { ?>
                                ...
                                <?php if ($row->status == "0"){ ?>
                                    <div class="flex justify-center items-center">
                                    <a href="<?= site_url('admin/invoice/cancel/'. $row->order_id)?>" class="btn btn-sm btn-danger text-white">Cancel</a>
                                    </div>   
                                <?php } else if ($row->status == "2") { ?>
                                    <button class="btn btn-sm btn-danger text-white">Payment Cancelled</button>
                                    <?php } ?>       
                                </div>
                          ...
                          <!--  <td class="table-report__action"><center>
                                ...
                                <?php } else if ($row->status == "1"){ ?>
                                    <button class="btn btn-sm btn-success text-white">Confirm</button>
                                <?php } else($row->status == "2") ?>
                            ...
                            <!--<td class="table-report__action"><center>
                                ...
                                <?php } else if ($row->status == "1"){ ?>
                                    <button class="btn btn-sm btn-success text-white" href="href="<?= site_url('admin/invoice/confirm/'. $row->order_id) ?> i data-lucide="arrow-left-right" class="w-4 h-4 mr-1">Payment Successfully</button>
                                <?php } else($row->status == "2") ?>
                                    <button class="btn btn-sm btn-danger text-white" href="href="<?= site_url('admin/invoice/cancel/'. $row->order_id) ?> i data-lucide="arrow-left-right" class="w-4 h-4 mr-1"> Cancel </button>
                            </td>-->
